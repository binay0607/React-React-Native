...
import {cybersecuritys} from '../../redux/actions';
...
const CyberSecurity = () => {
  ...
  var cybersecuritysdata = useSelector(state => state.cybersecuritys);
  const currentPage = useSelector(state => state.cybersecuritys.currentPage);
  const [loadingbtn, setLoadinbtn] = useState(false);
  const [filteredData, setFilteredData] = useState([]);
  useEffect(() => {
    console.log('currentPage1', currentPage);
    setLoadinbtn(true);
    dispatch(cybersecuritys(currentPage));
  }, []);

  useEffect(() => {
    if (cybersecuritysdata.is_success) {
      setLoadinbtn(false);
      setFilteredData(cybersecuritysdata?.cybersecuritys_data);
    } else if (cybersecuritysdata.error) {
      setLoadinbtn(false);
    } else {
    }
  }, [cybersecuritysdata]);
  ...
  const handleEndReached = (currentPage: number) => {
    setLoadinbtn(true);
    dispatch(cybersecuritys(parseInt(currentPage) + 1));
  };
  const renderFooter = () => {
