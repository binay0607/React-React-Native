...
import {movers} from '../../redux/actions';
...
const MoversShakers = () => {
  ...
  var moversdata = useSelector(state => state.movers);
  const currentPage = useSelector(state => state.movers.currentPage);
  const [loadingbtn, setLoadinbtn] = useState(false);
  const [filteredData, setFilteredData] = useState([]);
  useEffect(() => {
    console.log('currentPage1', currentPage);
    setLoadinbtn(true);
    dispatch(movers(currentPage));
  }, []);

  useEffect(() => {
    if (moversdata.is_success) {
      setLoadinbtn(false);
      setFilteredData(moversdata?.movers_data);
    } else if (moversdata.error) {
      setLoadinbtn(false);
    } else {
    }
  }, [moversdata]);
  ...
  const handleEndReached = (currentPage: number) => {
    setLoadinbtn(true);
    dispatch(movers(parseInt(currentPage) + 1));
  };
  const renderFooter = () => {
