...
import {Startup} from '../../redux/actions';
...
const StartUp = () => {
  ...
  var Startupdata = useSelector(state => state.Startup);
  ...
  useEffect(() => {
    ...
    setLoadinbtn(true);
    dispatch(Startup(currentPage));
  }, []);

  useEffect(() => {
    if (Startupdata.is_success) {
      setLoadinbtn(false);
      setFilteredData(Startupdata?.startup_data);
    } else if (Startupdata.error) {
      setLoadinbtn(false);
    } else {
    }
  }, [Startupdata]);
  ...
  const handleEndReached = (currentPage: number) => {
    setLoadinbtn(true);
    dispatch(Startup(parseInt(currentPage) + 1));
  };
  const renderFooter = () => {
